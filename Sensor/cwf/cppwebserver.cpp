...
CppWebServer::CppWebServer(const Configuration &configuration, Filter *filter) : configuration(configuration), filter(filter)
{
    ...
    defineThreadPriority();
}

void CppWebServer::defineThreadPriority()
{
    int maxThread   = configuration.getMaxThread();
    int idealThread = QThread::idealThreadCount();
    if(idealThread >= maxThread)
    {
        priority = QThread::NormalPriority;
    }
    else
    {
        int times = maxThread / idealThread;
        if(times >= 0 && times <= 10)
        {
...
void CppWebServer::incomingConnection(qintptr socketfd)
{
    ...
    pool.start(new HttpReadRequest(socketfd, urlController, sessions, configuration, sslConfiguration, filter), priority);
}
